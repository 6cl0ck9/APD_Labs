...
with col1:
    # Вибір параметра
    selected_param = st.selectbox("Оберіть параметр", ["VCI", "TCI", "VHI"], key="selected_param")
    
    # Вибір області (region_id)
    region_ids = df['region_id'].unique()
    selected_region = st.selectbox("Оберіть область (region_id)", options=region_ids, key="region_ids")
    
    # Слайдер для вибору інтервалу тижнів
    week_range = st.slider("Виберіть інтервал тижнів", 1, 52, (1, 52), key="week_range")
    
    # Слайдер для вибору інтервалу років
    year_range = st.slider("Виберіть інтервал років", int(df["Year"].min()), int(df["Year"].max()),
...
with col2:
    ...
    with tab3:
        comparison_df = df[(df["Year"].between(*year_range)) & (df["Week"].between(*week_range))]
        comparison_df["highlight"] = comparison_df["region_id"].apply(lambda x: "Обрана область" if x == selected_region else "Інші області")
        fig, ax = plt.subplots()
